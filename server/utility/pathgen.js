function pathgen(mongoose,hbs,paths,res){
    ...
    if(usr[i].length===1){usr[i]="0"+usr[i]};
        ...
        mansec.readDb({id:data[0]}).then((data)=>{
            data="../"+data;
            return genHTML(data,"",++n,data[0]);
        }).then((data)=>{
           return mansec.readDb({id:data[1]}).then((pt)=>{
               pt="../"+pt;
               return  genHTML(pt,data,++n,data[1]);
            })
        }).then((data)=>{
            return mansec.readDb({id:data[2]}).then((pt)=>{
                pt="../"+pt;
                return  genHTML(pt,data,++n,data[2]);
             })
        }).then((data)=>{
            return mansec.readDb({id:data[3]}).then((pt)=>{
                pt="../"+pt;
                return  genHTML(pt,data,++n,data[3]);
             })
        }).then((data)=>{
           append(data);
        }).catch((err)=>{
    ...
    function append(res){
        res.render("../../resources/views/master/main.hbs", {
            body: data
        });
